    ...
    def load_model(self):
        if __name__ == '__main__':
            ...
            try:
                ...
                model = keras.models.load_model(val_saves[-1], custom_objects={'absolute_error': losses.absolute_error})
        ...
        if train_flag == 'test':
            ...
            for j in range(int(test_size)):
                ...
                try:
                    ...
                    print("end of file, padding with 0s")
                    print(len(sample[()]['lidar']))
                    print(j)
                    img_batch[j, :len(sample[()]['lidar']), :, :] = sample[()]['lidar']
                    label_batch[j, :len(sample[()]['lidar']), :, :] = sample[()]['label']
